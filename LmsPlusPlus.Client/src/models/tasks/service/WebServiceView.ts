...
export class WebServiceView implements ServiceView, Disposable {
    ...
    dispose(): void {
        this.unmount()
        if (WebServiceView._iframeContainer.contains(this._iframe))
            WebServiceView._iframeContainer.removeChild(this._iframe as HTMLIFrameElement)
    ...
    mount(element: HTMLElement): void {
        ...
        if (!this._iframe) {
            this._iframe = WebServiceView.createIframe(this._virtualPort)
            WebServiceView._iframeContainer.appendChild(this._iframe)
        }
